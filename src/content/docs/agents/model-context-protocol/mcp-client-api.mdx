...
- **`ready`** — Fully connected with all capabilities discovered and available
- **`failed`** — Connection failed (check observability events for error details)

Use this method to monitor connection status, list available tools, or build UI for connected servers.

:::note[Connection State Changes]

Starting from version 0.0.24, the connection flow includes a new `connected` state to distinguish between transport connection and capability discovery. This allows for better control over when tool discovery occurs.
